      ...
      <div className="container-narrow relative z-10 pb-16 pt-16 md:pb-24 md:pt-20">
        ...
        <div className="mx-auto mt-14 grid max-w-4xl gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {entryCards.map((card, i) => (
            <motion.div key={card.title} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 + i * 0.1 }}>
              <Link to={card.to} onClick={() => onCtaClick?.(`card_${card.title.toLowerCase().replace(/\s/g, "_")}`)} className="group block rounded-xl border border-primary-foreground/8 bg-primary-foreground/5 p-5 backdrop-blur-sm transition-all duration-300 hover:border-gold/30 hover:bg-primary-foreground/10">
